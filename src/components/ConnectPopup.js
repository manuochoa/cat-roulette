...
export default function ConnectPopup({ popupVisible, setPopupVisible }) {
    ...
    return (
        <Popup className="popup--connect" popupShow={popupVisible} setPopupShow={setPopupVisible}>
            ...
            <button className={"popup__button" + (checkboxes.metamask ? " active" : "")} onClick={() => setCheckboxes({ metamask: true, walletConnect: false })}>
                <img src={metamaskIcon} className="popup__button-icon" alt="metamask" />
                <h1 className="popup__button-title">Metamask</h1>
                <span className="popup__button-text">Connect to your MetaMask Wallet</span>
            </button>
            <button className={"popup__button" + (checkboxes.walletConnect ? " active" : "")} onClick={() => setCheckboxes({ metamask: false, walletConnect: true })}>
                <img src={wcIcon} className="popup__button-icon" alt="wallet connect" />
                <h1 className="popup__button-title">WalletConnect</h1>
                <span className="popup__button-text">Scan with WalletConnect to connect</span>
            </button>
        </Popup>
    );
}
