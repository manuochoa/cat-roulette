...
export default function Quantity({ value, setValue, className }) {
    ...
    return (
        <div className={"quantity " + (className ? className : "")}>
            ...
            <input type="number" className="quantity__value" value={value} onChange={handleChange} />
            <button className="quantity__button quantity__button--2" onClick={increase}></button>
        </div>
    )
}
